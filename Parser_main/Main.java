...
class Main {
	...
	public static void customCode() {
		...
		Word testString = new Word("1*x+0");
		System.out.println(LINE + "\nTest 1:\n" + LINE + "\n");
		System.out.println("Does in-built grammar generate the string " + testString);
		...
		// chcp 65001;
		int count = 0;
		Word test = new Word("(x(*1");
		// Word test = new Word("011");
		ContextFreeGrammar cfg = MyGrammar.makeGrammar();
		// ContextFreeGrammar cfg = ContextFreeGrammar.simpleCNF();
		System.out.println(String.format("%s is in the language: %b", test.toString(), parser.isInLanguage(cfg, test)));
		 ...
		 * 
		 * 
		 * Rule r01 = new Rule(S, new Word(S, plus, T));
		 * Rule r02 = new Rule(T, new Word(T, multiply, F));
		 * Rule r03 = new Rule(S, new Word(T));
		 * Rule r04 = new Rule(T, new Word(F));
		 * Rule r05 = new Rule(F, new Word(left, S, right));
		 * Rule r06 = new Rule(F, new Word(Unit));
		 * Rule r07 = new Rule(F, new Word(Zero));
		 * Rule r08 = new Rule(F, new Word(x));
		 * 
		 ...
		 * rules2.add(r07);
		 * rules2.add(r08);
		 * ContextFreeGrammar cfg2 = new ContextFreeGrammar(rules2);
		 * 
		 * System.out.println(cfg2.equals(cfg));
		 * 
		 * System.out.println(cfg);
		 * System.out.println(cfg2);
		 */

		// Word testString = new Word("1");
	...
	public static void main(String[] args) {
		if (!SKIP_TO_TESTS) {
			System.out.println(
					"|------- Welcome to the demo script. I recommend turning on word wrap in your console. -------|\n"
	...
	private static void demoCode() {
		...
		System.out
				.println("The class Word represents strings. We use the name Word because String is already taken.\n");
		...
		System.out.println("\t     new Word(\"A0\"); \t\t-- this produces: " + new Word("A0")
				+ " (variable followed by terminal)");
		System.out.println();
		...
		System.out
				.println("The constructor takes a symbol object, plus zero, one, or two ParseTreeNodes as children.\n");
